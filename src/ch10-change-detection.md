...
溢出保护是 u32 选择的必然后果。如果使用 u64 Tick，以 144fps、每帧 1000 次递增计算，溢出需要约 4 亿年——实际上永远不会发生。但 u64 会使每个 ComponentTicks 从 8 字节增加到 16 字节，在百万实体场景中多消耗数十 MB 内存。Bevy 选择 u32 + 溢出保护的策略，以一次约每小时的全 World 扫描（将过旧的 Tick 钳位）换取每个组件 8 字节的紧凑存储。扫描本身的开销与组件总数成正比，但由于每个组件只需一次整数比较和条件赋值，即使在百万级组件的场景中也只需几毫秒。这种设计体现了 Bevy 在内存效率和偶发计算开销之间的务实权衡。
...
UI 布局计算（Taffy/Flexbox）只在节点样式变更时重新计算：

```rust
// UI layout only recalculates when style changes
fn ui_layout_system(
    changed_nodes: Query<(Entity, &Node, &Style), Changed<Style>>,
) { ... }
```
...
| UI 布局 | `Changed<Style>` | 只重算变化的节点 |
| 渲染提取 | `Changed<Material>` | 只重传变化的资源 |
| PBR 光照 | `Changed<Light>` | 只更新变化的光源 |
| Visibility | `Changed<Visibility>` | 只重算变化的可见性 |

变更检测的规模效应尤为显著。在一个拥有 10 万个实体的场景中，如果每帧只有 100 个实体的 Transform 发生变化，`Changed<Transform>` 过滤器让传播系统只需处理 0.1% 的实体。这不仅节省了 CPU 时间，还极大地改善了缓存效率——只访问少量分散的数据比扫描整个 Table 更友好。然而，`Changed<T>` 过滤器本身也有开销：它需要检查每个匹配 Archetype 中每个实体的 `changed_tick`。当变更比例接近 100% 时（例如所有实体每帧都在移动），使用 `Changed<T>` 反而比不使用更慢，因为多了一次 Tick 比较的开销。因此，变更检测最适合"稀疏变更"的场景——大量静态实体中少量发生变化。对于每帧必然变更的数据（如帧计数器），使用变更检测过滤器反而是一种浪费。这种性能特征与第 15 章的 Transform 传播系统直接相关：静态场景中的大量不变实体是变更检测最大的受益者。
